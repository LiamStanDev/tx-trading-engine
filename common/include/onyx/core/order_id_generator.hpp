#ifndef ONYX_CORE_ORDER_ID_GENERATOR_HPP
#define ONYX_CORE_ORDER_ID_GENERATOR_HPP
...
//   void reset(uint64_t start = 1) noexcept { counter_.store(start, std::memory_order_relaxed); }
//
//   /// @brief 取得當前計數器值（不消耗 ID）
//   /// @return 當前計數器值
//   /// @note 僅供除錯使用，多執行緒下可能立即過期
//   uint64_t current() const noexcept { return counter_.load(std::memory_order_relaxed); }
// };
