#ifndef ONYX_IO_TCP_SOCKET_HPP
#define ONYX_IO_TCP_SOCKET_HPP

#include "onyx/io/socket.hpp"
#include "onyx/io/socket_address.hpp"
...
class TcpSocket {
 ...
 public:
  ...
  static Result<TcpSocket> connect(const SocketAddress& remote_addr, bool nodelay = true) noexcept;
  /// @brief 建立 TCP Server 監聽
  /// @param local_addr 本地綁定地址
  /// @param backlog 連線佇列大小（預設 128）
  /// @return TcpSocket（處於監聽狀態）或錯誤
  static Result<TcpSocket> serve(const SocketAddress& local_addr, int backlog = 128) noexcept;
  ...
  // ==========================
  /// @brief 發送數據
  Result<size_t> send(std::span<const std::byte> data) noexcept { return socket_.send(data); }

  /// @brief 接收數據
  Result<size_t> recv(std::span<std::byte> buffer) noexcept { return socket_.recv(buffer); }

  // ==========================
  // Socket 選項
  // ==========================
  /// @brief 設定 TCP_NODELAY（禁用 Nagle 算法）
  Result<> set_nodelay(bool enable) noexcept { return socket_.set_tcp_nodelay(enable); }

  /// @brief 設定 SO_KEEPALIVE（啟用 TCP Keepalive）
  Result<> set_keepalive(bool enable) noexcept { return socket_.set_tcp_keepalive(enable); }

  /// @brief 設定非阻塞模式
  Result<> set_nonblocking(bool enable) noexcept { return socket_.set_nonblocking(enable); }
