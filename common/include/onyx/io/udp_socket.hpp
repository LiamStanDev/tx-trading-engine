#ifndef ONYX_IO_UDP_SOCKET_HPP
#define ONYX_IO_UDP_SOCKET_HPP
...
class UdpSocket {
 ...
 public:
  ...
  ///  - 32: 同一地區
  ///  - 255: 全球
  Result<> set_multicast_ttl(int ttl = 1) noexcept { return socket_.set_multicast_ttl(ttl); }
  ...
  Result<size_t> sendto(std::span<const std::byte> data, const SocketAddress& dest) noexcept {
  ...
  Result<size_t> recvfrom(std::span<std::byte> buffer, SocketAddress* src = nullptr) noexcept {
  ...
  Result<> set_recv_buffer_size(int size) noexcept { return socket_.set_recv_buffer_size(size); }

  /// @brief 設定發送緩衝區大小
  /// @return 成功或錯誤
  Result<> set_send_buffer_size(int size) noexcept { return socket_.set_send_buffer_size(size); }
