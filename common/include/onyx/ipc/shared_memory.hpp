...
class SharedMemory {
 private:
  ...
  explicit SharedMemory(std::string name, void* addr, size_t size, int fd, bool owner,
                        bool huge_page) noexcept
      : name_(std::move(name)),
        addr_(addr),
        size_(size),
        fd_(fd),
 ...
 public:
  ...
  static Result<SharedMemory> create(std::string name, size_t size, mode_t mode = 0600) noexcept;
