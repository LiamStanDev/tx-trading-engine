...
namespace onyx::io {
Result<TcpSocket> TcpSocket::connect(const SocketAddress& remote_addr, bool nodelay) noexcept {
...
Result<TcpSocket> TcpSocket::serve(const SocketAddress& local_addr, int backlog) noexcept {
  auto socket = TRY(Socket::create_tcp());
