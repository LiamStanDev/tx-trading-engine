#include "onyx/ipc/shared_memory.hpp"
...
Result<SharedMemory> SharedMemory::create(std::string name, size_t size, mode_t mode) noexcept {
  if (name.empty() || name[0] != '/') {
    return onyx::fail(std::errc::invalid_argument, "SHM should start with '/'");
  }

  if (size == 0) {
    return onyx::fail(std::errc::invalid_argument, "Invalid size");
  }
  ...
  if (fd < 0) {
    return onyx::fail(errno, "shm_open() failed");
  }
  ...
  if (::ftruncate(fd, static_cast<off_t>(size)) < 0) {
    ...
    return onyx::fail(saved_errno, "ftruncate() failed");
  }
  ...
  if (addr == MAP_FAILED) {
    ...
    ::shm_unlink(name.c_str());
    return onyx::fail(saved_errno, "mmap() failed");
  }
...
Result<SharedMemory> SharedMemory::create_huge(std::string name, size_t size,
  ...
  if (fd < 0) {
    return onyx::fail(errno, "open() failed");
  }
  ...
  void* addr =
      ::mmap(nullptr, actual_size, PROT_READ | PROT_WRITE, MAP_SHARED | MAP_POPULATE, fd, 0);
  if (addr == MAP_FAILED) {
    int saved_errno = errno;
    ::close(fd);
...
Result<SharedMemory> SharedMemory::open(std::string name) noexcept {
  ...
  if (::fstat(fd, &file_stat) < 0) {
    ...
    return onyx::fail(saved_errno, "fstat() failed");
  }
