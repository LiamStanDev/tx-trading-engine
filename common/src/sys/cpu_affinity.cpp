...
Result<std::vector<size_t>> read_available_cpus_from_sysfs() {
  std::vector<std::byte> bytes =
      TRY(io::File::open("/sys/devices/system/cpu/online", O_RDONLY).and_then([](io::File f) {
        return f.read_to_end();
      }));
...
Result<> CPUAffinity::pin_to_cpu(size_t cpu_id) noexcept {
  if (!is_valid_cpu(cpu_id)) {
    return onyx::fail(std::errc::invalid_argument, "CPU ID out of range");
  }
  ...
  if (sched_setaffinity(0, sizeof(cpuset), &cpuset) == -1) {
    return onyx::fail(errno, "sched_setaffinity() failed");
  }
...
Result<> CPUAffinity::pin_to_cpus(std::span<const size_t> cpu_ids) noexcept {
  if (cpu_ids.empty()) {
    return onyx::fail(std::errc::invalid_argument, "Empty CPU list");
  }
  ...
  for (size_t cpu_id : cpu_ids) {
    if (!is_valid_cpu(cpu_id)) {
      return onyx::fail(std::errc::invalid_argument, "Invalid CPU ID in list");
    }
    CPU_SET(cpu_id, &cpuset);  // 設置該 cpu id
  }
...
Result<std::vector<size_t>> CPUAffinity::get_affinity() noexcept {
  ...
  if (sched_getaffinity(0, sizeof(cpuset), &cpuset) == -1) {
    return onyx::fail(errno, "sched_getaffinity() failed");
  }
...
bool CPUAffinity::is_valid_cpu(size_t cpu_id) noexcept { return cpu_id < get_cpu_count(); }
...
bool CPUAffinity::is_cpu_available(size_t cpu_id) noexcept {
  ...
  return std::find(available.begin(), available.end(), cpu_id) != available.end();
}
