...
TEST(FileTest, OpenNonExistentFile_ReturnsENOENT) {
  ...
  EXPECT_FALSE(file);
  EXPECT_EQ(file.error(), std::errc::no_such_file_or_directory);
}
...
TEST(FileTest, OpenDirectory_ReturnsEISDIR) {
  ...
  // 嘗試以寫入模式開啟目錄應該失敗
  if (!result) {
    EXPECT_EQ(result.error(), std::errc::is_a_directory);
  }
}
...
TEST(FileTest, CreateTemp_GeneratesValidFile) {
  ...
  EXPECT_TRUE(file->path().string().starts_with("/tmp/test-"));

  // 清理
  ::unlink(file->path().c_str());
}

TEST(FileTest, MoveSelfAssignment_Safe) {
  TempFile temp;
  ASSERT_TRUE(temp.is_valid());

  auto file = File::open(temp.path(), O_RDONLY).value();
  int original_fd = file.fd();

// 忽略編譯器對自我移動的警告，因為我們就是要測試這種邊界情況
#pragma GCC diagnostic push
#pragma GCC diagnostic ignored "-Wself-move"
  file = std::move(file);
#pragma GCC diagnostic pop
  EXPECT_TRUE(file.is_open());
  EXPECT_EQ(file.fd(), original_fd);
}
...
TEST(FileTest, ReadEntireFile) {
  ...
  std::string read_content(reinterpret_cast<const char*>(buffer.data()), buffer.size());
...
TEST(FileTest, OperationsOnClosedFile) {
  TempFile temp;
  auto file = File::open(temp.path(), O_RDWR).value();
  file.close();

  std::byte b;
  EXPECT_EQ(file.read({&b, 1}).error(), std::errc::bad_file_descriptor);
  EXPECT_EQ(file.write({&b, 1}).error(), std::errc::bad_file_descriptor);
  EXPECT_EQ(file.size().error(), std::errc::bad_file_descriptor);
  EXPECT_EQ(file.seek(0).error(), std::errc::bad_file_descriptor);
  EXPECT_EQ(file.sync().error(), std::errc::bad_file_descriptor);
}
...
TEST(FileTest, ReadWriteCycle) {
  ...
  std::string read_back(reinterpret_cast<const char*>(buffer.data()), buffer.size());
...
TEST(FileTest, ReadExact_ValidBufferSize) {
  TempFile temp;
  temp.write_content("123");
  auto file = File::open(temp.path(), O_RDONLY).value();

  std::vector<std::byte> buffer(3);
  auto result = file.read_exact(buffer);

  ASSERT_TRUE(result);
  std::string res(reinterpret_cast<char*>(buffer.data()), buffer.size());
  EXPECT_EQ(res, "123");
}

TEST(FileTest, ReadExact_UnexpectedEOF) {
  TempFile temp;
  temp.write_content("123");
  auto file = File::open(temp.path(), O_RDONLY).value();
...
TEST(FileTest, ReadToEnd_FromStart) {
  TempFile temp;
  temp.write_content("0123456789");

  auto file = File::open(temp.path(), O_RDONLY).value();

  auto result = file.read_to_end();

  ASSERT_TRUE(result);
  EXPECT_EQ(result->size(), 10);
  EXPECT_EQ(reinterpret_cast<char*>(result->data())[0], '0');
}

TEST(FileTest, ReadToEnd_FromMiddle) {
  TempFile temp;
  temp.write_content("0123456789");
  auto file = File::open(temp.path(), O_RDONLY).value();

  ASSERT_TRUE(file.seek(5));
  auto result = file.read_to_end();

  ASSERT_TRUE(result);
  EXPECT_EQ(result->size(), 5);
  EXPECT_EQ(reinterpret_cast<char*>(result->data())[0], '5');
}

TEST(FileTest, ReadToString) {
  TempFile temp;
  std::string content = "Hello String World";
  ASSERT_TRUE(temp.write_content(content));

  auto file = File::open(temp.path(), O_RDONLY).value();
  auto result = file.read_to_string();
