...
#include "onyx/io/tcp_socket.hpp"
...
TEST_F(SocketTest, Bind_Success) {
  ...
  ASSERT_TRUE(result) << "bind failed: " << result.error().context().message;
}
...
TEST_F(SocketTest, TCP_ConnectAccept_Success) {
  ...
  std::thread client_thread([port]() {
    ...
    auto result = client->connect(SocketAddress::from("127.0.0.1", port).value());
...
TEST_F(SocketTest, TCP_SendRecv_Success) {
  ...
  // Server
  auto server = TcpSocket::serve(SocketAddress::any(port), 1);
  ASSERT_TRUE(server);
  ...
  // 驗證收到的資料
  std::string received(reinterpret_cast<char*>(received_data.data()), received_data.size());
...
TEST_F(SocketTest, UDP_RecvfromWithoutSource) {
  ...
  auto bytes = std::as_bytes(std::span(message));
  ASSERT_TRUE(sender->sendto(bytes, SocketAddress::from("127.0.0.1", port).value()));
...
TEST_F(SocketTest, JoinMulticastGroup_ValidAddress) {
  ...
  ASSERT_TRUE(result) << "join_multicast_group failed: " << result.error().message();
}
