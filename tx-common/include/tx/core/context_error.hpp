#ifndef TX_TRADING_ENGINE_CORE_CONTEXT_ERROR_HPP
#define TX_TRADING_ENGINE_CORE_CONTEXT_ERROR_HPP
...
class ContextError {
 private:
  ...
  explicit ContextError(std::error_code code) noexcept : code_(code) {}

  /// @brief 私有建構子（僅供工廠方法使用）
  explicit ContextError(std::error_code code, std::string context) noexcept
 ...
 public:
  ...
  static ContextError from_errc(std::errc ec) noexcept;

  /// @brief 從 std::errc 建立錯誤並附加上下文
  /// @param ec 標準錯誤條件枚舉
  /// @param context 錯誤上下文描述
  /// @return 包含該錯誤條件與上下文的 Error
  static ContextError from_errc(std::errc ec, std::string context) noexcept;

  // ===========================
  // RAII
  // ===========================
  ContextError(const ContextError& other) = delete;
  ContextError& operator=(const ContextError& other) = delete;
  ContextError(ContextError&& other) = default;
  ContextError& operator=(ContextError&& other) = default;
