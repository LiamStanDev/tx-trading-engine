...
class TcpSocket {
 ...
 public:
  ...
  static SocketResult<TcpSocket> connect(const SocketAddress& remote_addr,
  ...
  static SocketResult<TcpSocket> serve(const SocketAddress& local_addr,
  ...
  SocketResult<TcpSocket> accept(SocketAddress* client_addr = nullptr) noexcept;
  ...
  SocketResult<> set_nodelay(bool enable) noexcept {
    return socket_.set_tcp_nodelay(enable);
  }

  /// @brief 設定 SO_KEEPALIVE（啟用 TCP Keepalive）
  SocketResult<> set_keepalive(bool enable) noexcept {
  ...
  /// @brief 取得遠端地址
  [[nodiscard]] SocketResult<SocketAddress> remote_address() const noexcept {
