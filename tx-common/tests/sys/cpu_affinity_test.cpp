...
TEST_F(CPUAffinityTest, IsValidCPU_BoundaryCheck) {
  ...
  EXPECT_TRUE(CPUAffinity::is_valid_cpu(count - 1))
      << "Last CPU should be valid";

  // Invalid cases
  EXPECT_FALSE(CPUAffinity::is_valid_cpu(-1))
      << "Negative CPU should be invalid";
  EXPECT_FALSE(CPUAffinity::is_valid_cpu(count))
      << "CPU ID == count should be invalid";
  EXPECT_FALSE(CPUAffinity::is_valid_cpu(9999))
      << "Large CPU ID should be invalid";
}
...
TEST_F(CPUAffinityTest, PinToCPU_Success) {
  ...
  ASSERT_TRUE(result) << "pin_to_cpu should successed: "
...
TEST_F(CPUAffinityTest, PinToCPU_ThreadLocal) {
  ...
  EXPECT_EQ((*main_affinity)[0], main_cpu)
      << "Main thread affinity should not change";

  // 驗證子線程成功綁定
  ASSERT_EQ(thread_affinity.size(), 1);
  EXPECT_EQ(thread_affinity[0], thread_cpu)
      << "Child thread should be pinned to different CPU";
}
...
TEST_F(CPUAffinityTest, PinToCPUs_Success) {
  ...
  ASSERT_TRUE(result) << "pin_to_cpus should succeed: "
                      << result.error().message();
  ...
  // 驗證包含目標 CPUs
  EXPECT_NE(std::find(affinity->begin(), affinity->end(), targets[0]),
            affinity->end());
  EXPECT_NE(std::find(affinity->begin(), affinity->end(), targets[1]),
            affinity->end());
}
...
TEST_F(CPUAffinityTest, ClearAffinity_RestoresAllCPUs) {
  ...
  ASSERT_TRUE(result) << "clear_affinity should succeed: "
                      << result.error().message();

  // 驗證恢復到所有 CPU
  auto affinity = CPUAffinity::get_affinity();
  ASSERT_TRUE(affinity);
  EXPECT_EQ(affinity->size(), CPUAffinity::get_cpu_count())
      << "Should be able to run on all CPUs";
}
